...
ROWS = [
    ("host",),
    ("kv", "Project", "ChaseBird (Micro-SaaS platform for freelancers)"),
    ("kv", "Stack", "Python, FastAPI, SvelteKit, Tailwind CSS"),
    ("kv", "Handle", "space998"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">space998@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">space998</tspan><tspan fill="{MUTED}">@</tspan>'
